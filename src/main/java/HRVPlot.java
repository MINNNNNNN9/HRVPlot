...
public class HRVPlot {
     ...
     */
    public void getFivePowerPlot(double heart, double health, double sex, double vital, double fight) {
        // 將輸入的百分比值轉換為 0-1 的比例
        double[] values = {heart / 100, health / 100, sex / 100, vital / 100, fight / 100};
        String[] labels = {
                "Heart " + (int) heart,
                "Health " + (int) health,
                "Sex " + (int) sex,
        ...
        g.dispose();
        try {
            ImageIO.write(image, "PNG", new File("fivepower.png"));
            System.out.println("五力圖已保存為 fivepower.png");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
     ...
     * 繪製太極圖並保存為 PNG 檔案
     */
    public void getTaiChiPlot(double ratio, int age) {
        int width = 800;
        int height = 800;
        ...
        g.dispose();
        try {
            ImageIO.write(image, "PNG", new File("taichi.png"));
            System.out.println("太極圖已保存為 taichi.png");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
